/**
 * Seed case studies (only when table is empty).
 * Run: node scripts/seed-case-studies.cjs
 */
...
async function main() {
  const existing = await prisma.caseStudy.count()
  if (existing > 0) {
    console.log(`CaseStudy table already has ${existing} rows — skipping seed.`)
    await prisma.$disconnect()
    return
  }

  let total = 0
  for (const [locale, cases] of Object.entries(casesByLocale)) {
  ...
  console.log(`Seeded ${total} CaseStudy rows.`)
  await prisma.$disconnect()
}
