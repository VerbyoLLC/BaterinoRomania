...
export default function HomePageClient({ initialProducts }: { initialProducts: PublicProduct[] }) {
  ...
  /** Starts null on server and first client render (hydration-safe); real value read after mount. */
  const [userType, setUserType] = useState<'profesionist' | 'client' | null>(null)
  /** Same: auth role lives in localStorage, so it's only known after mount. */
  const [viewerRole, setViewerRole] = useState<ReturnType<typeof getAuthRole>>(null)

  useEffect(() => {
    const stored = sessionStorage.getItem(USER_TYPE_STORAGE_KEY)
    if (stored === 'profesionist' || stored === 'client') setUserType(stored)
    setViewerRole(getAuthRole())
  }, [])
        ...
        <section id="produse-section" className="mb-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 sm:mb-10">
            ...
            featuredProducts.map((p) => {
              ...
              const to = getCatalogProductHrefForViewer(p, viewerRole)
